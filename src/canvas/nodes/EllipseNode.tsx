...
export function EllipseNode({ data, selected }: NodeProps<Node<NodeData>>) {
  return (
    <div className={`dm-node dm-ellipse${selected ? " is-selected" : ""}`}>
      ...
      <span className="dm-label">{data.label || "Ellipse"}</span>
    </div>
  );
}
